...
class ChatBotOracle:
    ...
    def submit_answer(self, answer: str, prompt_id: int, address: str):
        ...
        try:
            ...
            print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Answer tx submitted. Waiting for receipt... Tx Hash: 0x{tx_hash.hex()}", flush=True)
            ...
            print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Answer tx confirmed. Hash: 0x{tx_receipt.transactionHash.hex()}", flush=True)
